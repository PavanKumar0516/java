...
import com.capgemini.pecuniafinance.exception.InvalidException;
...
public class AccountManagement {
	...
	public static void main(String[] args) {
		...
		IAccountManagementService  serviceobj = new AccountManagementServiceImp();
		...
		try {
			...
			while (true) {
				...
				boolean flags = AccountManagementServiceImp.checkAccNumber(accNumber);
				boolean flag1 = AccountManagementServiceImp.validAccountNumber(account);
				if (flag1 == true && flags == true) {
					...
					case 1:
						while (true) {
							...
							case 1:
								...
								boolean flag = AccountManagementServiceImp.validName(name);
								if (flag == true) {
							...
							case 2:
								...
								boolean flag2 = AccountManagementServiceImp.validMobileNumber(phno);
								if (flag2 == true) {
							...
							case 3:
								...
								AccountManagementServiceImp.validAddressLine1(address1);
								System.out.println("Please enter customer address2");
								String address2 = scanner.next();
								AccountManagementServiceImp.validAddressLine2(address2);
								System.out.println("Please enter customer city");
								String city = scanner.next();
								AccountManagementServiceImp.validAddressCity(city);
								System.out.println("Please enter customer state");
								String state = scanner.next();
								AccountManagementServiceImp.validAddressState(state);
								System.out.println("Please enter customer country");
								String country = scanner.next();
								AccountManagementServiceImp.validAddressCountry(country);
								System.out.println("Please enter customer pincode");
								int pincode = scanner.nextInt();
								AccountManagementServiceImp.validAddressPincode(pincode);
								boolean flag3 = AccountManagementServiceImp.validAddress(address1, address2, city, state, country,
										pincode);
								if (flag3 == true) {
					...
					try {
							throw new InvalidException("Invalid Account Number or Account Number not exist in database... ");	
